...
/// run takes a string argument to search and a Hashmap of replacements.
    ...
    for (c_index, c) in from.char_indices() {
        ...
        if !all_matches.is_empty() {
            ...
            all_matches = all_matches
                .into_iter()
                .filter(|matcher| match matcher.status {
                    Status::Failed => false,
                    Status::Running => true,
...
impl StringCache {
    ...
    fn inc_index_by(&mut self, num: usize) {
        self.start_i += num;
    }
...
fn hash_to_vec_ordered_longest_shortest<'a>(
    seqs_hash: HashMap<&'a str, &'a str>,
) -> Vec<(&'a str, &'a str)> {
...
impl<'a> Matcher<'a> {
    fn new(from_seq: &'a str, to_seq: &'a str, start_i: usize) -> Self {
...
mod tests {
    ...
    fn highest_complexity() {
        ...
        let new_s = run(from_s, replacement_seqs);
        println!("Orig: {}", from_s);
    ...
    #[test]
    fn longest_collapse_three_shorters() {
        let mut replacement_seqs: HashMap<&str, &str> = HashMap::new();
    ...
    #[test]
    fn longest_collapse_two_shorters() {
        let mut replacement_seqs: HashMap<&str, &str> = HashMap::new();
        replacement_seqs.insert("wow is my fox", "mmmmmmmmm");
        replacement_seqs.insert("is", "bb");
        replacement_seqs.insert("my", "cc");
    ...
    #[test]
    fn longest_collapse_replace_shorter() {
        let mut replacement_seqs: HashMap<&str, &str> = HashMap::new();
